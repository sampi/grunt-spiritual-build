 ...
 * @extends {ts.gui.Model}
 */
ts.gui.CalendarModel = (function() {
	...
	return ts.gui.Model.extend({
		...
		onconstruct: function() {
			ts.gui.Model.prototype.onconstruct.call(this);
			if (ts.lib.Date) {
				this._currentDay = ts.lib.Date.getCurrentDay();
				this._selectedDate =
						ts.lib.Date.dateStringToObject(this.now || 'today');
		...
		_generateLabels: function(year, month) {
			...
			for (i = 0; i < 7; ++i) {
				dayLabels.push(
						ts.gui.CalendarModel.DAY_NAMES_MIN[
						(i + ts.gui.CalendarModel.firstDay) % 7
						]
				);
			}
			return dayLabels;
					...
					if (day > 0 && day <= numDays) {
						...
						tempRow[col] = new Cell({
							...
							selected: selectedIsVisible &&
									day === parseInt(this._selectedDate.day, 10),
							year: year,
							month: month,
							day: day
						});
		 ...
		 */
		regenerate: function() {
			this.month = ts.gui.CalendarModel.MONTH_NAMES_SHORT[this._month];
